	...
	public class Selector : MonoBehaviour {
		...
		private void Run2DRaycast() {
			if (raycastAll) {
				...
				Debug.DrawRay(CustomPosition, new Vector3(5,5,0));
				
				//Debug.Log ("1");
				//Vector3 testing = CustomPosition;
				//Debug.Log (CustomPosition.GetType());
		...
		private void Run3DRaycast() {
			...
			if (raycastAll) {
				...
				ray = new Ray(CustomPosition, new Vector3(xDir * 1, yDir * 1, 0)); // for custom positioning
				Debug.DrawRay(CustomPosition, new Vector3(xDir * 1, yDir * 1, 0));
